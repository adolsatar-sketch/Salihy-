 ...
 * descendant (AmbientBackground's own layers, any future decorative
 * component) can read the same values with plain `var(--scroll-shift,
 * 0px)` in a style, without each one running its own scroll/pointer
 * listener. Call this exactly once (AmbientBackground, mounted once in
 * the root layout, owns it) — a single rAF loop, not one per consumer.
 */
